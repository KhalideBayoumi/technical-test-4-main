      ...
      <div className="flex flex-wrap gap-5 p-2 md:!px-8">
        <SelectProject
          ...
          onChange={(selectedProject) => {
            // Check if a project has been selected and use its _id, otherwise use an empty string for "All Projects"
...
const Activities = ({ date, user, project }) => {
  ...
  useEffect(() => {
    (async () => {
      // Build the query string based on the presence or absence of a selected project
      let queryString = `/activity?date=${date.getTime()}&user=${user.name}`;
      if (project) {
        queryString += `&projectId=${project}`;
      }

      const { data } = await api.get(queryString);
      const projectsData = await api.get(`/project/list`);
      
      setActivities(
        data.map((activity) => ({
          ...activity, 
          projectName: projectsData.data.find((p) => p._id === activity.projectId)?.name
        }))
      );
      setOpen(null);
    })();
  ...
  async function onSave() {
    ...
    if (activities.length >= 2) {
      ...
      toast.success(`Saved ${activities[0].projectName}`);
    }
  }
  ...
  async function onDelete(i) {
    if (window.confirm("Are you sure ?")) {
      ...
      setActivities(prevActivities => prevActivities.filter((_, index) => index !== i)); // Update status to exclude deleted activity
  ...
  const getTotal = () => {
    return (activities.reduce((acc, a) => acc + a.total, 0)).toFixed(2);
  };
                          ...
                          <th className="w-[100px] border-t border-b border-r text-[12px] font-bold text-[#212325] text-left">
                            <div className="flex flex-1 items-center justify-between gap-1 px-2">
                              ...
                              <div className="flex flex-col items-end">
                                <div className="text-xs italic font-normal">{(e.total).toFixed(2)} days</div>
                                <div className="text-[10px] italic font-normal">{e.total.toFixed(2) > 0 ? ((e.total.toFixed(2) / getTotal()) * 100).toFixed(2) : 0}%</div>
