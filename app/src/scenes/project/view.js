                          ...
                          <tr className="border-t border-b border-r border-[#E5EAEF]" key={`1-${e._id}`}>
                            <th className="w-[100px] border-t border-b border-r text-[12px] font-bold text-[#212325] text-left">
                              <div className="flex flex-1 items-center justify-between gap-1 px-2">
                                ...
                                <div className="text-md italic font-normal">{(e.total).toFixed(2)} days</div>
                              </div>
                            </th>
                            {e.detail.map((f, j) => {
...
const Links = ({ project }) => {
  // Function to check and prefix the URL with http:// if necessary
    ...
    <div className="flex flex-wrap gap-3">
      {project.website && (
        <div className="group text-sm font-medium	text-gray-700 border-[1px] border-gray-700 rounded-full overflow-hidden">
          <a target="blank" href={ensureHttpPrefix(project.website)} className="break-words cursor-pointer text-gray-700 hover:text-white hover:bg-gray-700 flex hover:no-underline h-full">
      ...
      {project.links?.map((link) => (
        <div className="group text-sm font-medium	text-blue-700 border-[1px] border-blue-700 rounded-full overflow-hidden">
          <a target="blank" href={ensureHttpPrefix(link.url)} className="break-words cursor-pointer text-blue-700 hover:text-white hover:bg-blue-700 flex hover:no-underline h-full">
